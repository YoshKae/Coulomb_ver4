function iflag = check_lonlat_info2
% グローバル変数に設定された緯度と経度の情報が適切に定義されているかをチェックし、その結果を iflag というフラグで返します。
% 出力：iflag = 1 （全ての情報が設定されている場合）, iflag = 0 （いずれかの情報が不足している場合）
global COORD_VARS
...
elseif isempty(COORD_VARS.MIN_LON)==1 | isempty(COORD_VARS.MAX_LON)==1
    iflag = 0;
% ゼロポイントの緯度または経度が設定されていない場合
elseif isempty(COORD_VARS.ZERO_LAT)==1 | isempty(COORD_VARS.ZERO_LON)==1
    iflag = 0;
