...
% temp_rate_change = zeros(length(YGRID),length(XGRID));
...
switch reply
    case 'y'
        ...
        tinc = str2double(input('  Time icrement for sensitivity:   day (integer e.g.,   300)?','s'));
...
%        minr = str2double(input('  minimum rate for first period: km (e.g., 0.000001)?','s'));
    ...
    otherwise
        ...
        n1 = int32((evttime - mintime)/tinc);
        n2 = int32((maxtime - evttime)/tinc);
        n3 = n1 + 1 + n2;
        rate_change = zeros(length(YGRID)*length(XGRID),n3);
...
rateAdj = length(find(mintime <= eq_datenum & eq_datenum < chpttime)) / sum(rot90(sum(rate1)));
rate1 = rate1 * rateAdj / double(t1);
...
for l = 1:nx                % x loop
    for m = 1:ny            % y loop
        rate2(m,l) = 0;
        for k = 1:neq       % EQ loop
            if eq_datenum(k) >= chpttime && eq_datenum(k) <= maxtime
                dist = sqrt((EQ_DATA(k,16)-xs(l))^2+(EQ_DATA(k,17)-ys(m))^2);
                if dist < r
...
rateAdj = length(find(eq_datenum >= chpttime & eq_datenum < maxtime)) / sum(rot90(sum(rate2)));
rate2 = rate2 * rateAdj / double(t2);

temp_rate_change = log10(rate2 ./ rate1);
    undefined1 = rate1 == minr;
    undefined2 = rate2 == 0.0;
    undefined  = (undefined1-1).*(undefined2-1);
    indund     = find(undefined == 0);
temp_rate_change(indund) = NaN;

rate_change(:,n)=reshape(temp_rate_change,[length(YGRID)*length(XGRID),1]);

end

%======= Find timing of maximized rate change ===========================


neq = n1 + 1;
% neq = neq + 1;
g_rate_change = reshape(rate_change(:,neq),[length(YGRID),length(XGRID)]);

%======= making image for undefined area =================================
figure('Name','Undefined area','NumberTitle','off');
hold on;
ac = image(-isnan(g_rate_change).*1.0,'CDataMapping','scaled','XData',[XGRID(1) XGRID(end)],...
...
hold on;
ac = image(g_rate_change,'CDataMapping','scaled','XData',[XGRID(1) XGRID(end)],...
