...
load('MyColormaps3','ANATOLIA_gray_end'); 

iflag = 0; % flag if this script reads input file (1) or using dialog (0)
try
...
disp(' ');
% try
%     load ('seis_rate_change_input.mat');    % for .mat file
%     disp('seis_rate_change_input.mat is read');
%     iflag = 1;
%     mintime = datenum(t1y,t1m,t1d);
%     evttime1 = datenum(t2y,t2m,t2d,t2h,t2n,0);
%     evttime2 = datenum(t3y,t3m,t3d,t3h,t3n,0);
%     maxtime = datenum(t4y,t4m,t4d);
% catch
    ...
    switch reply
        case 'y'
            ...
            t3y  = str2double(input('  Event time2:    year  (integer e.g., 2001)?','s'));
            t3m  = str2double(input('  Event time2:    month (integer e.g.,    4)?','s'));
            t3d  = str2double(input('  Event time2:    day   (integer e.g.,   21)?','s'));
            t3h  = str2double(input('  Event time2:   hour   (integer e.g.,    5)?','s'));
            t3n  = str2double(input('  Event time2:   minute (integer e.g.,   56)?','s'));
        ...
        case 'n'
            ...
            t4y  = str2double(input('  Ending time:   year  (integer e.g., 2003)?','s'));
            t4m  = str2double(input('  Ending time:   month (integer e.g.,   12)?','s'));
            t4d  = str2double(input('  Ending time:   day   (integer e.g.,   31)?','s'));
            t2y  = str2double(input('  Event time1:    year  (integer e.g., 2000)?','s'));
            t2m  = str2double(input('  Event time1:    month (integer e.g.,    4)?','s'));
            t2d  = str2double(input('  Event time1:    day   (integer e.g.,   21)?','s'));
            t2h  = str2double(input('  Event time1:   hour   (integer e.g.,    5)?','s'));
            t2n  = str2double(input('  Event time1:   minute (integer e.g.,   56)?','s'));
            t3y  = str2double(input('  Event time2:    year  (integer e.g., 2001)?','s'));
            t3m  = str2double(input('  Event time2:    month (integer e.g.,    4)?','s'));
            t3d  = str2double(input('  Event time2:    day   (integer e.g.,   21)?','s'));
            t3h  = str2double(input('  Event time2:   hour   (integer e.g.,    5)?','s'));
...
rateAdj = length(find(mintime <= eq_datenum & eq_datenum < evttime1)) / sum(rot90(sum(rate1)));
...
for l = 1:nx                % x loop
    for m = 1:ny            % y loop
        rate2(m,l) = 0;
        for k = 1:neq       % EQ loop
            if eq_datenum(k) >= evttime2 && eq_datenum(k) <= maxtime
                dist = sqrt((EQ_DATA(k,16)-xs(l))^2+(EQ_DATA(k,17)-ys(m))^2);
                if dist < r
...
rateAdj = length(find(eq_datenum >= evttime2 & eq_datenum < maxtime)) / sum(rot90(sum(rate2)));
...
ac = image(rate_change,'CDataMapping','scaled','XData',[XGRID(1) XGRID(end)],...
            'YData',[YGRID(1) YGRID(end)]);colormap(ANATOLIA_gray_end);        
...
if iflag == 1 % skip the following dialog when an input file read.
    return
end
