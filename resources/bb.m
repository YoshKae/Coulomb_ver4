...
if D > 0
   ...
   if ta == 0
      fill(xx,yy,'w')
      fill(Y,X,color)
      line(xx,yy,'color','k','linewidth',0.1);
   else
      fillm(yy,xx,'w')
      fillm(X,Y,color)
      linem(yy,xx,'color','k','linewidth',0.1);
   end
else
   if ta == 0
      fill(X,Y,color)
...
% draw radial circles
    ...
    for i=(rmin+rinc):rinc:rmax
        hhh = plot(xunit*i,yunit*i,ls,'color',tc,'linewidth',0.1,...
                   'handlevisibility','off');
...
%    cs = [-cst; cst];
%    sn = [-snt; snt];
%    plot(rmax*cs,rmax*sn,ls,'color',tc,'linewidth',0.1,...
%         'handlevisibility','off')
