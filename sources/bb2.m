function [hg] = bb3(fm, centerX, centerY, diam, ta, iFlag, color)
...
%    penc = 1.1; % one nodal plane for coulomb calc.
    penc = 0.3; % one nodal plane for coulomb calc.
...
% *** Originally implemented 'AuxPlane' has a big trouble (bug!) **
% Get azimuth and dip of second plane
% [s2,d2,r2] = AuxPlane(s1,d1,r1);
% format short
% [fm(:,1) fm(:,2) fm(:,3) s1 d1 r1 s2 d2 r2]

[outsdr] = nodal_plane_calc(s1,d1,r1);
s1 = outsdr(:,1);
d1 = outsdr(:,2);
r1 = outsdr(:,3);
s2 = outsdr(:,4);
...
%       hg = hggroup;
      ...
      h2 = fill(Y,X,color); % when simple color (e.g., 'r')
                 ...
                 set(h2,'Parent',hg);
      if iFlag == 1
        hold on; h3 = plot(Yc1,Xc1,'Color',[0.1,0.1,0.1],'linewidth',penc);  
